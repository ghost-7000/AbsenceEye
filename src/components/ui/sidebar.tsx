...
>(
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn("hidden md:flex h-full flex-col text-sidebar-foreground bg-sidebar-background transition-all duration-200 ease-in-out", 
...
>(({ className, onClick, children, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      {...props}
    >
      {children || (open && !isMobile ? <PanelLeft/> : <PanelRight />)}
    </Comp>
  )
})
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-semibold data-[active=true]:text-sidebar-accent-foreground data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground group-data-[collapsible=icon]:group-data-[state=collapsed]:h-10 group-data-[collapsible=icon]:group-data-[state=collapsed]:w-10 group-data-[collapsible=icon]:group-data-[state=collapsed]:justify-center group-data-[collapsible=icon]:group-data-[state=collapsed]:p-0 [&>span]:group-data-[collapsible=icon]:group-data-[state=collapsed]:hidden [&>span]:whitespace-nowrap [&>span]:truncate [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
